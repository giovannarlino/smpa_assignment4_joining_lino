...
6)  After joining both datasets, I want to find the amount of wins for each party in each state. 
#This set of codes allows me to compare the amount of democratic vs republican winners in each state and which state had a major republican vs democratic vote. 
...
mergedtables <- inner_join(counties_elexresults, counties_demographics) 

inner_join(mergedtables, counties_rural_urban_suburban, by = NULL, copy = FALSE)
